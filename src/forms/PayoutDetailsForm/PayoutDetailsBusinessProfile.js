...
const PayoutDetailsBusinessProfile = props => {
  const { fieldId, disabled, intl, country, showBusinessURLField, showMCCForUSField } = props;

  const isBusinessProfileNeeded = showBusinessURLField || showMCCForUSField;
  const mccLabel = intl.formatMessage({ id: 'PayoutDetailsForm.mccForUSLabel' });
  const mccPlaceholder = intl.formatMessage({ id: 'PayoutDetailsForm.mccForUSPlaceholder' });
  const mccRequired = validators.required(
    intl.formatMessage({ id: 'PayoutDetailsForm.mccForUSRequired' })
  );
